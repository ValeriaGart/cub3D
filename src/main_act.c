...
void	ft_put_pixel_small(t_data *data, int x, int y, int colour)
{
	...
	(void)y;
	(void)x;
	//offset = ((y * WIDTH_SMALL) + x) * 4;//y * ((WIDTH_SMALL * 2) + (HEIGHT_SMALL * 2)) + (x * (data->small_w.bits_per_pixel / 8));
	//offset = (y * (HEIGHT_SMALL / data->map->y_map)) * (x * (WIDTH_SMALL / data->map->x_map));
...
void    ft_small_w(t_data **data)
{
	...
	ft_put_pixel_small(*data, (*data)->plr.x * (WIDTH_SMALL / (*data)->map->x_map), (*data)->plr.y * (HEIGHT_SMALL / (*data)->map->y_map), 0xEE4B2B);
