...
void	ft_rotate(t_rc *raycast, char direction)
{
	...
	if (direction == 'l')
	{
		...
		raycast->dirY = old_dir_x * sin(0.3) + raycast->dirY * cos(0.3);
		old_plane_x = raycast->planeX;
		raycast->planeX = raycast->planeX * cos(0.3)
			- raycast->planeY * sin(0.3);
		raycast->planeY = old_plane_x * sin(0.3) + raycast->planeY * cos(0.3);
	}
	if (direction == 'r')
	{
		old_dir_x = raycast->dirX;
