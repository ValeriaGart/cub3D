...
void	ft_init_raycast_loop(t_data *data, int x)
{
	...
	data->raycast.rayDirX = data->raycast.dirX + data->raycast.planeX * data->raycast.cameraX;
	data->raycast.rayDirY = data->raycast.dirY + data->raycast.planeY * data->raycast.cameraX;
	...
	if (data->raycast.rayDirX == 0)
		...
		data->raycast.deltaDistX = sqrt(1 + (data->raycast.rayDirY * data->raycast.rayDirY) / (data->raycast.rayDirX * data->raycast.rayDirX));
	if (data->raycast.rayDirY == 0)
		data->raycast.deltaDistY = 1e30;
	else
		data->raycast.deltaDistY = sqrt(1 + (data->raycast.rayDirX * data->raycast.rayDirX) / (data->raycast.rayDirY * data->raycast.rayDirY));
	data->raycast.hit = 0;
}
